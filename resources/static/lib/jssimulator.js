...
async function getSimulations(identifier) {
    const rep = await fetch(BACKEND_URL + "simulations/" + identifier).then(response => response.json());
}
...
async function buildSimulatorWindow(event) {
    ...
    let metadata = await fetch(BACKEND_URL + "metadata/" + modelNumber).then(response => response.json());

    // get simulations of currently selected model
    const simulations = await fetch(BACKEND_URL + "simulations/" + modelNumber).then(response => response.json());
    ...
    // Add simulations
    let simulationSelect = document.getElementById("simulationSelect");
    simulationSelect.innerHTML = simulations.map(simulation => `<option>${simulation.name}</option>`).join();
...
async function getExecutionResult(modelNumber){
    const rep = await fetch(BACKEND_URL + "execute/" + modelNumber + "?simulationIndex=" + _selectedSimulationIndex);
